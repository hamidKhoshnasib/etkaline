...
export function AuthDialog({ trigger }: AuthDialogProps) {
  ...
  const [loading, setLoading] = React.useState<"captcha" | "login" | "verify" | "resend" | null>(
    null,
  );

  const normalizedMobile = toEnglishDigits(mobile).replace(/\D/g, "");
  const mobileIsValid = /^09\d{9}$/.test(normalizedMobile);
  ...
  async function handleVerify(verificationCode: string) {
    if (verificationCode.length !== OTP_LENGTH || loading) {
      return;
    }
