...
function BrandCard({ name, image, href = "#" }: Brand) {
  return (
    <Link
      href={href}
      className="flex h-20 w-full shrink-0 items-center justify-center rounded-[20px] border border-[#CBD5E1] sm:h-26.25 sm:w-[102.5px]"
    >
...
export default function PopularBrands({ brands }: PopularBrandsProps) {
  return (
    <section className="w-full rounded-[28px] border border-[#E2E8F0] bg-white p-4">
      ...
      <div className="grid grid-cols-3 gap-3 sm:flex sm:gap-4 sm:overflow-x-auto sm:pb-1 [&::-webkit-scrollbar]:hidden">
        {brands.map((brand) => (
